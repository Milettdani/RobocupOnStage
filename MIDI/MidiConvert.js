  ...
  */
   ...
   */
		...
		value: function decode(bytes) {
			...
			this.tempo = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_4__Header__["b" /* parseTempo */])(midiData, this.header);
			this.timeSign = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_4__Header__["c" /* parseTimeSignature */])(midiData, this.header);
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return parseTempo; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return parseTimeSignature; });
...
function parseTempo(midiJson, header) {
	var ret = [];
	var t = 0;
	var absoluteTime = 0;
	for (var i = 0; i < midiJson.tracks.length; i++) {
		...
		for (var j = 0; j < track.length; j++) {
			var datum = track[j];
			t += datum.deltaTime;
			absoluteTime += __WEBPACK_IMPORTED_MODULE_0__Util__["a" /* ticksToSeconds */](datum.deltaTime, header);
			if (datum.type === "meta") {
				if (datum.subtype === "timeSignature") {
					ret.timeSignature = [datum.numerator, datum.denominator];
				} else if (datum.subtype === "setTempo") {

					ret.push({ "absoluteTime": t, "seconds": absoluteTime, "bpm": 60000000 / datum.microsecondsPerBeat });
				}
			}
		}
	}
...
function parseTimeSignature(midiJson, header) {
	var ret = [];
	var t = 0;
	var absoluteTime = 0;
	for (var i = 0; i < midiJson.tracks.length; i++) {
		...
		for (var j = 0; j < track.length; j++) {
			...
			if (datum.type === "meta") {
				if (datum.subtype === "timeSignature") {
					console.log(datum);
					ret.push({ "absoluteTime": t, "seconds": absoluteTime, "numerator": datum.numerator, "denominator": getBaseLog(2, datum.denominator), "click": datum.metronome, "notesQ": datum.thirtyseconds });
				} else if (datum.subtype === "setTempo") {}
			}
		}
	}
...
var Track = function () {
   ...
   */
		...
		value: function encode(trackEncoder, header) {
			...
			__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_2__Merge__["a" /* Merge */])(this.noteOns.sort(function (a, b) {
